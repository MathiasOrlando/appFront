...
export class ConsultarLibroComponent implements OnInit {
  ...
  displayedColumns: string[] = ['codigo', 'descripcion','cantidad','obs', 'actions'];
  dataSource: any = [];
  ...
  consultar = () => {
    axios.get<any>("http://localhost:8080/PROYECTO-REST/rest/libro/list").then(resultado => {
  ...
  borrar = (codigo: number) => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
    axios.delete<any>(`http://localhost:8080/PROYECTO-REST/rest/libro/delete/${codigo}`).then(resultado => {
